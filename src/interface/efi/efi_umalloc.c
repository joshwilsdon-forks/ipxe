 ...
 * memory block.
 *
 * There is a Joyent hack here: we limit allocations to below 2.5Gb so dboot can
 * find space above the boot modules to do allocations on some memory layouts.
 * See OS-8138 for details.
 */
static userptr_t efi_urealloc ( userptr_t old_ptr, size_t new_size ) {
	EFI_BOOT_SERVICES *bs = efi_systab->BootServices;
	EFI_PHYSICAL_ADDRESS phys_addr = 0x9c400000UL;
